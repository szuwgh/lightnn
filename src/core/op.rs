...
impl Op {
    pub(crate) fn infer(&self, inputs: Tensors) -> LNResult<Tensors> {
        match self {
            ...
            Op::MaxPool(t) => Ok(smallvec![reshape(inputs)?]),
...
#[derive(Debug)]
pub(crate) struct MaxPool2D((usize, usize), (usize, usize));

trait Map {
    fn func<T>(&self, src: &[T], shape: &Dim) -> Vec<T>;

    fn map(&self, t: Tensor) -> GTensor {
        match t.as_value_ref().as_tensor() {
            GTensor::U8(t1) => GTensor::U8(self.func(t1.as_slice(), t1.shape())),
...
impl MaxPool2D {
    fn func<T: TensorType>(&self, src: &[T], shape: &Dim) -> LNResult<Vec<T>> {
        // https://pytorch.org/docs/stable/generated/torch.nn.MaxPool2d.html
        let (k_h, k_w) = self.0;
        let (s_h, s_w) = self.1;
        let (b_sz, c, h, w) = shape.dim4()?;
...
// impl MaxPool2D {
//     fn apply(inputs: Tensors) -> LNResult<Tensor> {
//         let a1 = args_1(inputs);
